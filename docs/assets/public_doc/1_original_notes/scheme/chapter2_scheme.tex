...
There is an equivalent notion of local property. Since GW did not discuss it, Vakil only briefly mentioned it, below is my attempt to properly define local property: A property $P$ on some collection of objects is a subset of the collection, if an object $X$ belong to the subset, we said $X$ has property $P$ or $X$ is $P$ or $P(X)$
...
If we replace open affine subsets by open set, we say that $P$ is open-local. However,  the open affine sets generate the topology on $X$, it is easy to show that open-local and affine-local are equivalent. It is also clear that stalk-local implies affine-local.
...
\begin{enumerate}
	\item A sheaf $\ms{F}$ on $X$ is completely determined by its étale space $E = \coprod_{x \in X} \ms{F}_x$ \footnote{topologized by the weakest topology generated by all compatible germs $(s_x)_{x \in U}$ for some $s \in \ms{F}(U)$}
	
	\item A section $s \in \ms{F}(U)$ is zero if and only if $s$ is zero on all stalks on $U$
\end{enumerate}

Back to local property, when $X = \Spec A$, then being zero is stalk-local.

\note{(I will mumble something that I am not an expert in, so this is likely wrong)}
Another subtle issue is these local definitions use higher order logic.
...
\begin{definition}[reduced scheme, integral scheme]
	A scheme is called reduced if every stalk is reduced. A scheme is called integral if it is reduced and irreducible.
\end{definition}
