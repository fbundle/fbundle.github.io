...
\begin{remark}[motivation of left derived functor]
    \note{something related to extending $FA \to FB \to FC \to 0$ to $... \to L_2 F A \to L_2 F B \to L_2 F C \to L_1 F A \to L_1 F B \to L_1 F C \to FA \to FB \to FC \to 0$ so that if $F$ is an exact functor (both left and right exact) then $L_1 F = 0$, that is, $L_1$ measures how far $F$ from being exact. In case of $\Tor$, $\Tor_1$ measures how far $\otimes M$ from being exact, or how far $M$ from being projective/free}
\end{remark}
