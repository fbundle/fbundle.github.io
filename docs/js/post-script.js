 ...
 */
async function highlightTab(highlight_id = "highlight", highlight_attr = "navbar_elem_id") {
 ...
 */
async function setupDropdownBehavior() {
...
async function setupMobileMenu() {
    const mobileToggle = document.getElementsByClassName('mobile-menu-toggle')[0];
    ...
    if (mobileToggle && navMenu) {
        ...
        mobileToggle.addEventListener('click', function () {
            ...
            if (navMenu.classList.contains('active')) {
                navMenu.style.top = 'var(--navbar-height)'; // Slide down to show
            }
            else {
                navMenu.style.top = '-100vh'; // Slide up to hide
            }
        });
    }
    ...
    document.addEventListener('click', function (e) {
        ...
        if (!target.closest('.navbar')) {
            ...
            if (navMenu) {
                ...
                navMenu.style.top = '-100vh'; // Hide menu
            }
 ...
 */
async function setupResponsiveBehavior() {
 ...
 */
async function setupHoverEffects() {
    const navMenu = document.getElementsByClassName('nav-menu')[0];
    if (!navMenu)
        return;
