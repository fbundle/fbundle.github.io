...
def get_pdf_desc(pdf_paths: list) -> tuple[str, dict]:
    """
    Generate descriptions for multiple PDF files and provide a summary.
    ...
    Raises:
        Exception: If there's an error processing the PDFs
    """
    if not pdf_paths:
    ...
    for pdf_path in pdf_paths:
        if not os.path.exists(pdf_path):
            descriptions[pdf_path] = "File not found"
            continue
            
        try:
            if PYMUPDF_AVAILABLE:
                description = _analyze_pdf_content(pdf_path)
            else:
                description = _generate_basic_description(pdf_path)
            
            descriptions[pdf_path] = description
            
            # Categorize and count
            category = _categorize_document(pdf_path, description)
    ...
    # Generate summary
    summary = _generate_collection_summary(categories, total_files, total_size)
...
if __name__ == "__main__":
    # Test the function
    test_paths = ["../docs/assets/public_doc/ma5204_notes.pdf"]
    if all(os.path.exists(path) for path in test_paths):
        summary, descriptions = get_pdf_desc(test_paths)
